...
def cluster_data(request):
    ...
    if run_id:
        try:
            ...
            return Response({"error": f"No completed run with id {run_id}"}, status=404)
    else:
...
def voter_cluster_membership(request):
    ...
    if "usuario" in voter_info and voter_info["usuario"]:
        ...
        return Response({"error": "Could not identify voter"}, status=400)

    # Find voter's projection
    try:
        projection = run.projections.get(voter_type=voter_type, voter_id=voter_id)
    except VoterProjection.DoesNotExist:
        return Response({"error": "Voter not found in clustering"}, status=404)
...
def cluster_voting_patterns(request, cluster_id):
    ...
    cluster = run.clusters.filter(cluster_id=cluster_id, cluster_type="group").first()

    if not cluster:
        cluster = run.clusters.filter(
            cluster_id=cluster_id, cluster_type="base"
        ).first()

    if not cluster:
        return Response({"error": f"Cluster {cluster_id} not found"}, status=404)
...
def trigger_clustering(request):
    ...
    logger.info(
        f"Triggering clustering: days={time_window_days}, min_voters={min_voters}"
    )
