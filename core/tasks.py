...
def update_voter_clusters(time_window_days=30, min_voters=10, min_votes_per_voter=3):
    ...
    try:
        ...
        old_runs = VoterClusterRun.objects.filter(
            status='completed'
        ).exclude(id=run.id).order_by('-created_at')[:5]  # Invalidate last 5 old runs
...
def generate_cluster_report_snapshot():
    ...
    try:
        ...
        if not run:
            logger.warning("No completed clustering run found, skipping report generation")
            return {
                'success': False,
                'run_id': None,
                'cache_key': None,
