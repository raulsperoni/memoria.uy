...
def update_voter_clusters(time_window_days=30, min_voters=10, min_votes_per_voter=3):
    ...
    try:
        ...
        for group_id, cluster_obj in group_cluster_obj_map.items():
            try:
                ...
                if name:
                    ...
                    logger.info(f"Group {group_id}: {name} ({cache_status})")
                else:
        ...
        # Invalidate old report snapshots since we have a new clustering run
        # The new snapshot will be generated by the hourly task
        logger.info("Invalidating old cluster report snapshots")
        old_runs = (
            VoterClusterRun.objects.filter(status="completed")
            .exclude(id=run.id)
            .order_by("-created_at")[:5]
        )  # Invalidate last 5 old runs

        for old_run in old_runs:
            old_cache_key = f"cluster_report_snapshot_{old_run.id}"
            cache.delete(old_cache_key)
            logger.debug(f"Deleted snapshot cache for old run {old_run.id}")

        # Trigger immediate snapshot generation for the new run
        # (in addition to hourly schedule)
        logger.info(f"Triggering immediate snapshot generation for new run {run.id}")
...
def send_reengagement_emails(
    ...
    for user in inactive_users:
        ...
        if user_cluster_name:
            html_lines.append(
                f"<p>Tu burbuja actual es: <strong>{user_cluster_name}</strong>.</p>"
            )
        html_lines.extend(
            [
                "<p>En cual estas?</p>",
    ...
    if notify_staff:
        ...
        if staff_emails:
            ...
            # HTML version
            staff_html_lines = [
                "<h2>Resumen de emails de reenganche enviados</h2>",
...
def send_daily_staff_summary():
    ...
    new_noticias = Noticia.objects.filter(fecha_agregado__gte=yesterday).order_by(
        "-fecha_agregado"
    )
    ...
    if new_noticias_count > 0:
        ...
        for noticia in new_noticias[:10]:  # Show up to 10 most recent
            ...
            html_lines.append(f'<li><a href="{noticia_url}">{title}</a></li>')
        html_lines.append("</ul>")
        if new_noticias_count > 10:
...
@shared_task
def generate_cluster_report_snapshot():
    """
    Generate and cache cluster report snapshot.

    This task calculates the expensive cluster report once per hour
    and stores it in cache. The view will serve from this snapshot
    instead of calculating in real-time.
    ...
    start_time = time.time()
    logger.info("Starting cluster report snapshot generation")
    ...
    try:
        ...
        if not run:
            logger.warning(
                "No completed clustering run found, skipping report generation"
            )
            return {
                "success": False,
                "run_id": None,
                "cache_key": None,
                "computation_time": 0,
                "error": "No completed clustering run",
            }

        cache_key = f"cluster_report_snapshot_{run.id}"

        # === EXECUTIVE SUMMARY ===
        logger.info(f"Calculating polarization score for run {run.id}")
        polarization_data = calculate_polarization_score(run)

        logger.info(f"Identifying bridge builders for run {run.id}")
        bridges = identify_bridge_builders(
            run, distance_threshold=0.5, min_connections=2
        )
        ...
        # === CONSENSUS SECTION ===
        logger.info(f"Calculating consensus news for run {run.id}")
        consensus_news = calculate_consensus_news(run, top_n=20)

        # === DIVISIVE SECTION ===
        logger.info(f"Calculating divisive news for run {run.id}")
        divisive_news = calculate_divisive_news(run, top_n=20)

        # === BRIDGES SECTION ===
        top_bridges = bridges[:25]  # Top 25 for display
        ...
        if recent_runs.exists():
            logger.info(f"Calculating polarization timeline for run {run.id}")
            polarization_timeline = calculate_polarization_timeline(recent_runs)

        # Prepare cache data
        cache_data = {
            "executive_summary": executive_summary,
            "consensus_news": consensus_news[:10],  # Top 10 for carousel
            "divisive_news": divisive_news[:10],
            "top_bridges": top_bridges,
            "bridge_stats": bridge_stats,
            "polarization_timeline": polarization_timeline,
            "generated_at": timezone.now().isoformat(),
            "run_id": run.id,
        }

        # Cache for 2 hours (safety margin beyond 1 hour schedule)
        cache.set(cache_key, cache_data, 7200)

        computation_time = time.time() - start_time
        logger.info(
            f"✓ Cluster report snapshot generated for run {run.id} "
        ...
        logger.error(f"Error generating cluster report snapshot: {e}", exc_info=True)
        return {
            "success": False,
            "run_id": None,
            "cache_key": None,
